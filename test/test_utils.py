...
# 添加模块路径
_scripts_python_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),'scripts','python')
...
class TestWindowsToolsUtils(unittest.TestCase):
    ...
    def test_scan_scripts(self):
        ...
        with patch('toolbox.utils.get_scripts_dir', return_value=self.mock_scripts_dir):
    ...
    def test_scan_scripts_priority(self):
        """测试扫描时优先保留ps1文件"""
        ...
        with patch('toolbox.utils.get_scripts_dir', return_value=self.mock_scripts_dir):
            scripts = toolbox.scan_scripts()
            paths = [s[0].lower() for s in scripts]
            self.assertTrue(any("dual.ps1" in p for p in paths))
            self.assertFalse(any("dual.bat" in p for p in paths))
    ...
    def test_scan_scripts_empty_dir(self):
        ...
        try:
            with patch('toolbox.utils.get_scripts_dir', return_value=empty_dir):
                scripts = toolbox.scan_scripts()
...
class TestToolsInterface(unittest.TestCase):
    ...
    def test_tools_interface_creation(self):
        with patch('toolbox.gui.main_window.scan_scripts', return_value=[]):
...
class TestWindow(unittest.TestCase):
    ...
    def test_window_close_cleanup(self):
        with patch('toolbox.gui.main_window.scan_scripts', return_value=[]):
            window = toolbox.Window()
            with patch('toolbox.gui.main_window.cleanup_tmp_dir') as mock_cleanup:
            ...
            with patch.object(task.process, 'state', return_value=QProcess.Running):
                with patch.object(task.process, 'terminate') as mock_terminate:
                    with patch.object(task.process, 'waitForFinished') as mock_wait:
                        ...
                        with patch('toolbox.gui.main_window.cleanup_tmp_dir'):
                            from PySide6.QtGui import QCloseEvent 
                            event = QCloseEvent()
                            window.closeEvent(event)
